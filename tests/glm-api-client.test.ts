...
describe('工具函数测试', () => {
  describe('isValidResolution', () => {
    it('应该返回 true 对于有效的分辨率', () => {
      expect(isValidResolution(1280, 1280)).toBe(true);
      expect(isValidResolution(1024, 1024)).toBe(true);
      expect(isValidResolution(512, 512)).toBe(true);
      expect(isValidResolution(2048, 2048)).toBe(true);
    });

    it('应该返回 false 对于无效的分辨率（太小）', () => {
      expect(isValidResolution(511, 512)).toBe(false);
      expect(isValidResolution(512, 511)).toBe(false);
    });

    it('应该返回 false 对于无效的分辨率（太大）', () => {
      expect(isValidResolution(2049, 2048)).toBe(false);
      expect(isValidResolution(2048, 2049)).toBe(false);
    });

    it('应该返回 false 对于不是 32 的整数倍的分辨率', () => {
      expect(isValidResolution(1281, 1280)).toBe(false);
      expect(isValidResolution(1280, 1281)).toBe(false);
      expect(isValidResolution(1000, 1000)).toBe(false);
    });
    ...
    it('应该正确处理边界值', () => {
      ...
      expect(isValidResolution(2048, 2048)).toBe(true);
      // 32 的倍数
      expect(isValidResolution(544, 576)).toBe(true);
    });
  });

  describe('calculateCost', () => {
    it('应该返回正确的 GLM-Image 成本', () => {
      expect(calculateCost('glm-image')).toBe(0.1);
    });

    it('应该返回正确的 CogView-4 成本', () => {
      expect(calculateCost('cogView-4-250304')).toBe(0.1);
    });

    it('应该返回默认成本对于未知模型', () => {
      expect(calculateCost('unknown-model' as ModelType)).toBe(0);
    });
  });
...
describe('类型定义测试', () => {
  it('ModelType 应该接受有效的模型值', () => {
    const models: ModelType[] = ['glm-image', 'cogView-4-250304'];
  ...
  it('分辨率预设应该包含所有标准尺寸', () => {
    ...
    expect(RESOLUTION_PRESETS[0]).toEqual(
      expect.objectContaining({ label: '1:1 (1280x1280)', width: 1280, height: 1280 })
    );
  });
...
describe('数据验证测试', () => {
  it('应该验证分辨率字符串格式', () => {
    ...
    validResolutions.forEach(res => {
      ...
      expect(isValidResolution(w, h)).toBe(true);
    });
  });
  ...
  it('应该计算正确的成本', () => {
    ...
    expect(calculateCost('cogView-4-250304')).toBe(0.1);
    // 批量计算
    const models: ModelType[] = ['glm-image', 'glm-image', 'cogView-4-250304'];
    const totalCost = models
      .reduce((sum, model) => sum + calculateCost(model), 0);
